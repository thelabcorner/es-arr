# Build ESARRArray.dll (the ESARR ExternalObject accelerator for ESPACK).
# FREESTANDING: no CRT - kernel32 imports only (own allocator + libc
# substitutes in esarr_array.c). clang+lld with the ArcFit/ESB64 family flags
# (-O3 -ffast-math -ffreestanding -fno-builtin -march=x86-64-v2
# -mtune=generic -flto, portable to any Windows x64 >= Win10 2015) when
# available; MSVC fallback (both link /nodefaultlib /entry:DllMain). The PE
# timestamp is fixed (/timestamp:0, or /Brepro for MSVC) so rebuilds are
# byte-identical - deterministic output, vendor drift guards can compare DLL
# bytes.
# Usage: powershell -ExecutionPolicy Bypass -File build.ps1 [-Name ESARRArray2.dll]
...
$dllName = "ESARRArray.dll"
if ($Name) { $dllName = $Name }

$src = Join-Path $here "esarr_array.c"
$obj = Join-Path $here "esarr_array.obj"
...
if ($clang -and $lld -and (Test-Path $clang) -and (Test-Path $lld)) {
    Write-Output "ESARRArray build: clang+lld (freestanding, x86-64-v2, -O3 -flto)"
    ...
    Write-Output "ESARRArray build: MSVC fallback (freestanding, /nodefaultlib)"
